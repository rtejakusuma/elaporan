...
class Api_sikd_model extends CI_Model
{
    ...
    public function get_tahunan($tahun = '2019')
    {
        ...
        foreach ($rawdata['data'] as $row) {
            ...
            $data['tw'] = array();
            array_push($data['tw'], ['realisasi' => $row['realisasi sd Tw 1'], 'target' => $row['target sd Tw 4'], 'prosentase' => $row['prosentase']]);
        }
        ...
        for ($i = 2; $i < 5; $i++) {
            ...
            $idx = 0;
            foreach ($rawdata['data'] as $row) {
                array_push($data['tw'], ['realisasi' => $row['realisasi sd Tw ' . $i], 'target' => $row['target sd Tw ' . $i], 'prosentase' => $row['prosentase']]);
                $idx++;
            }
        }
