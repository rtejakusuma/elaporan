...
class laporankegiatan extends CI_Controller
{
    ...
    public function export($formname, $id_laporan)
    {
        ...
        // ini atur header
        $sheet->setCellValue('A1', 'LAPORAN KEGIATAN DAN ANGGARAN')
            ->mergeCells('A1:D1');
        $sheet->setCellValue('A2', 'TAHUN ' . date('Y', strtotime($this->data['fetch']['laporankegiatan']['tgl'])) . ' DI LINGKUNGAN PEMERINTAH KOTA MADIUN')
            ->mergeCells('A2:D2');
        ...
        $sheet->setCellValue('C5', 'NILAI TAHUN ' . date('Y', strtotime($this->data['fetch']['laporankegiatan']['tgl'])));
        $sheet->setCellValue('D5', 'PREDIKAT');

        // td numrow 6
        $numrow = 7;
        $nomor = 0;
        foreach ($this->data['fetch']['laporankegiatanopd'] as $laporankegiatan) {
            $nomor++;
            $sheet->setCellValue('A' . $numrow, $nomor);
            $sheet->setCellValue('B' . $numrow, ucwords($laporankegiatan['nama_opd']));
            $sheet->setCellValue('C' . $numrow, $laporankegiatan['kode_rekening']);
            $sheet->setCellValue('D' . $numrow, $laporankegiatan['program']);
            $sheet->setCellValue('E' . $numrow, $laporankegiatan['uraian_kegiatan']);
            $sheet->setCellValue('F' . $numrow, $laporankegiatan['pagu_anggaran']);
            $numrow++;
        }
